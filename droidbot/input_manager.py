...
from .input_event import EventLog, SetTextEvent
...
class InputManager(object):
    ...
    def autoTest(self):
        listResult = [self.evaluate()]

        packageName = self.app.get_package_name()
        firstTestCase = listTestCase[0]

        for expectedIndex in range(1, len(listTestCase)):
            testCase = listTestCase[expectedIndex]

            self.device.adb.shell("am force-stop " + packageName)
            self.device.start_app(self.app)

            for event in self.events:
                eventTemp = deepcopy(event)
                if isinstance(event, SetTextEvent):
                    for indexValue in range(len(firstTestCase)):
                        if str(event.text) == str(firstTestCase[indexValue]):
                            value = testCase[indexValue]
                            if isinstance(value, float) and math.isnan(value):
                                value = ""
                            eventTemp.text = str(value)
                            break

                eventTemp.send(self.device)
                time.sleep(0.5)

            expectedIndex = 1
            expectedResult: str = originalExpected
            while expectedIndex > 0:
                strTemp = "@" + str(expectedIndex)
                if strTemp in expectedResult:
                    value = listTestCase[expectedIndex]
                    if isinstance(value, float) and math.isnan(value):
                        value = ""

                    expectedResult = expectedResult.replace(strTemp, str(value))
                    expectedIndex = expectedIndex + 1
                else:
                    break
            expected = expectedResult

            listResult.append(self.evaluate())

        csvBefore = pandas.read_csv(testCasePath)
        csvBefore["result"] = listResult
        csvBefore.to_csv(testCasePath.replace(".csv", "") + "_result.csv", index=False)

    def evaluate(self):
        state_ui, _, _, _ = self.device.get_current_state().get_described_actions()

        introduction = '''You are a smartphone feature tester. Given an expected result with the current UI state, your task is to decide whether the current result is pass or fail.'''
        task_prompt = 'Expected result: ' + expected
        full_state_prompt = 'Current UI state: \n' + state_ui
        request_prompt = "\nYour answer should only 1 if pass, 0 if otherwise"
        prompt = introduction + '\n' + task_prompt + '\n' + full_state_prompt + '\n' + request_prompt
