...
def query_gpt(prompt):
    gpt_url = "https://api.gpt4-all.xyz/v1"
    # gpt_key = "g4a-rUvrwwpPALQENRZVxDO6qNQt5xxMweWef1i"
    # gpt_key = "g4a-skdINITJK46I5u5gi7JTCnaLG1T29Q6Q3Nb"
    gpt_key = "g4a-xYK2VTSBmzjDZbYIOXH7CE9WIVf7XSltbdZ"
    client = OpenAI(
        base_url=gpt_url,
        api_key=gpt_key
    )
...
def extract_action(v):
    ...
    try:
        if 'Finished' in v.keys():
            whether_finished_answer = v['Finished'].lower() == 'yes' or v['Finished'].lower() == 'y' or v[
                'Finished'].lower() == 'true' or 'finished' in v['Finished'].lower()
